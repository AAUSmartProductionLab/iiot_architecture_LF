#!/usr/bin/env bash
# One-line installer for the IIoT edge gateway: clones (or updates) the repo and
# brings the gateway stack up via edge_gateway/redeploy.sh.
#
...
# --- build + start ----------------------------------------------------------
log "Building and starting the gateway stack"
cd "$TARGET_DIR/edge_gateway"
chmod +x redeploy.sh
./redeploy.sh

log "Gateway is up. Health check: curl http://localhost:8000/api/health"
